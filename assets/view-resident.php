...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'delete') {
        ...
        $query = "DELETE FROM residents WHERE id = '$resident_id'";
        if (mysqli_query($conn, $query)) {
            header("Location: view-resident.php?action=success&text=حذف ساکن با موفقیت انجام شد");
        } else {
            header("Location: view-resident.php?action=danger&text=" . mysqli_error($conn));
        }
    } elseif ($_GET['action'] == 'success') {
        $deleteResponse = ['success', $_GET['text']];
    } elseif ($_GET['action'] == 'danger') {
        $deleteResponse = ['danger', "Error: " . $_GET['text']];
    }
}

$query = "SELECT residents.*, apartments.apartment_number 
          FROM residents 
          INNER JOIN apartments ON residents.apartment_id = apartments.id";
$result = mysqli_query($conn, $query);
$adminSidebarMenuItems['view-resident']['is-active'] = true;
?>
                    ...
                    <div class="col-xxl-12 col-lg-12">
                        <div class="card mb-3">
                            <div class="card-header">
                                <h5 class="card-title">لیست ساکنین</h5>
                            </div>
                            <?php if (isset($deleteResponse)) : ?>
                                <div class="alert bg-<?= $deleteResponse[0] ?> alert-dismissible d-flex align-items-center fade show" role="alert">
                                    <?= $deleteResponse[0] == 'danger' ? '<i class="bi bi-x-circle fs-2 me-2 lh-1"></i>' : '<i class="bi bi-check-circle fs-2 me-2 lh-1"></i>' ?>
                                    <?= $deleteResponse[1] ?>
                                            ...
                                            <tbody>
                                                <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                                                    <tr>
                                                        ...
                                                        <td><?= $row['contact'] ?></td>
                                                        <td><?= ($row['is_primary_resident'] ? 'بله' : 'خیر') ?></td>
                                                        <td>
                                                            <a href="edit-resident.php?rId=<?= $row['id'] ?>" class="btn btn-outline-warning">ویرایش</a>
                                                            <a href="view-resident.php?action=delete&rId=<?= $row['id'] ?>" class="btn btn-outline-danger">حذف</a>
                                                        </td>
                                                    </tr>
                                                <?php endwhile; ?>
                                            </tbody>
